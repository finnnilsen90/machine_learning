...
def sim_pearson(prefs,p1,p2):
    ...
    # print('sum1Sq => ' + str(sum1Sq))
    # print('sum2Sq => ' + str(sum2Sq))
...
def getRecommendations(prefs,person,similarity=sim_pearson):
    ...
    for other in prefs:
        ...
        for item in prefs[other]:
            ...
            # only score moview I haven't seen yet 
...
def getRecommendedItems(prefs, itemMatch, user):
    ...
    for (item,rating) in userRatings.items():
        ...
        for (similarity,item2) in itemMatch[item]:
            ...
            # Weighted sum of rating itmes simiarity
...
def loadMovieLens(path='/data/movielens'):
    ...
    # Get movie titles 
    movies={}
    for line in open(full_path+'/u.item', encoding = "ISO-8859-1"):
        (id,title)=line.split('|')[0:2]
        movies[id]=title

    # Load data
    prefs={}
    for line in open(full_path+'/u.data'):
