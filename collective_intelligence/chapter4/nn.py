...
class searchnet:
    ...
    # Finds the strength between node connections. 
    ...
    # Sets the strength between node connections
    ...
    # Retrieves hidden node through hidden node id.
    ...
    # Find differences between input and outputs and update weights. 
    ...
    def updatedatabase(self):
        ...
        for i in range(len(self.wordids)):
            for j in range(len(self.hiddenids)):
                fromid=list(self.wordids)
                toid=list(self.hiddenids)
                self.setstrength(fromid[i],toid[j],0,self.wi[i][j])
        for j in range(len(self.hiddenids)):
            for k in range(len(self.urlids)):
                fromid=list(self.hiddenids)
                toid=list(self.urlids)
                self.setstrength(fromid[j],toid[k],1,self.wo[j][k])
        self.con.commit()